...
use crate::{config::activity_distribution::ActivityDistributionMatrix, constants::github, git_models::git_models::{CreateRef, CreateReviewRequest, UpdateFileRequest, UpdateFileResponse, UpdateRepo}};
    ...
    pub async fn create_pull_requests(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        */
        ...
        for pull_request_index in 0..self.activity_distribution.issues as u16 {
            ...
            let base_ref_value: serde_json::Value = self.octocrab.get(get_ref_url, None::<&()>).await?;
            ...
            let _create_branch_response: serde_json::Value = self.octocrab
            ...
            let pull_request = self.octocrab
            ...
            let _review_request_response: serde_json::Value = self.octocrab
                .post::<CreateReviewRequest, _>(review_request_url, Some(&review_request))
                .await?;

            // 5. Merge pull request
            let merge_response = self.octocrab
                .pulls(github::GIT_OWNER, github::GIT_REPO)
